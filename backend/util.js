...
function applyCollections(options, master, updates) {
  if (!options.dataRepo) {
    return Promise.reject(new Error('no proper options for "applyCollections" provided'));
  }
  if (!master || !updates) {
    return Promise.resolve();
  }
...
function prepareUpdates(options, data, cm, knownId) {
  if (!options.dataRepo || !options.metaRepo || !options.keyProvider) {
    return Promise.reject(new Error('no proper options for "prepareUpdates" provided'));
  }
  data = data || {};
  ...
  return savers
    .then(() => uploadFiles(options, cm.getCanonicalName(), knownId, uploads))
    .then((fileIds) => {
